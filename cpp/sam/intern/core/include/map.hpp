...
using Points3dArray = std::vector<Eigen::Vector3d, Eigen::aligned_allocator<Eigen::Vector3d>>;
...
struct Voxel{
    ...
    // %            ... convert a point into a voxel coordinates
    static Voxel coordinate(const Eigen::Vector3d& point, double voxel_size) {
        return {
...
struct VoxelBlock{
    ...
    Points3dArray points;                                   // list of 3D points contained within voxel
...
class map{
    public:
        // %            ... map constructor
        explicit map(int lifetime): lifetime(static_cast<int32_t>(lifetime)) {}
        ...
        [[nodiscard]] Points3dArray pointcloud() const {
            Points3dArray points;
            points.reserve(size());
        ...
        // %            ... remove far or expired voxel
        void remove(const Eigen::Vector3d &origin, double distance) {
        ...
        // %            ... adding point with potns3darray
        void add(const Points3dArray &points, double voxel_size, double min_distance_points, int voxel_capacity) {
            for (const auto &point : points) add(point, voxel_size, min_distance_points, voxel_capacity);
        }
        // %            ... adding point with vector point3d
        void add(const std::vector<Point3d> &points, double voxel_size, double min_distance_points, int voxel_capacity) {
            for (const auto &point : points) add(point.pointsMap, voxel_size, min_distance_points, voxel_capacity);
        }
        // %            ... adding single point to map
        void add(const Eigen::Vector3d &point, double voxel_size, double min_distance_points, int voxel_capacity) {
            Voxel key = Voxel::coordinate(point, voxel_size);
            auto search = map_.find(key);
            if (search != map_.end()) {
                auto &voxel_block = search.value();
                if (!voxel_block.isfull()) {
                    double sq_dist_min_to_points = voxel_size * voxel_size;
        ...
        std::tuple<Eigen::Vector3d, double> closestneighbor(const Eigen::Vector3d& query, double voxel_size) const {
            ...
            for (const auto& voxel_shift : shift) {
                ...
                if (search != map_.end() && !search->second.points.empty()) {
                    ...
                    double distance_sq = (neighbor - query).squaredNorm();
    ...
    private:
        const std::array<Voxel, 27> shift{{Voxel{0, 0, 0},   Voxel{1, 0, 0},   Voxel{-1, 0, 0},  Voxel{0, 1, 0},   Voxel{0, -1, 0},
